...
확인 완료된 게시글은 관리자만 안전하게 보관할 수 있고, 필요하면 게시판 데이터 관리에서 복원할 수 있습니다. 게시글·댓글·변경이력은 Google Sheets에 보존됩니다.

```toml
TEAM_ACCESS_PIN = "팀원 접근번호"
ADMIN_ACCESS_PIN = "관리자 접근번호"
```

접근번호는 GitHub 파일이 아니라 Streamlit Secrets에만 저장합니다. 이전 `REVIEW_BOARD_DELETE_PIN`은 관리자 전체 권한으로 사용하지 않으며, `ADMIN_ACCESS_PIN`을 별도로 설정합니다.
...
`성경 검색`은 외부 성경 서비스에 접속하지 않고 저장된 `bible_text.txt`의 텍스트값만 사용합니다. `창:1:1`, `창 1:1`, `창세기 1장 1절`, `행 7:2~3`, `사도행전 7장 2~3절` 형식과 여러 구절이 들어간 설교 문자를 인식합니다. `~`, `-`, `–`로 표시된 같은 장의 구절 범위는 각 절로 자동 확장하며 한 번에 최대 100개 구절을 처리합니다.

배포 루트에 `bible_text.txt`가 있으면 앱이 재부팅되어도 이를 기본 성경 본문으로 자동 연결합니다. 관리자 화면의 `성경 원본 교체`는 기본 파일을 덮어쓰지 않고 현재 접속에서만 다른 번역본을 사용합니다. 공개 저장소에 이 파일을 올리면 누구나 원문을 내려받을 수 있으므로, 내부 전용이라면 비공개 저장소나 별도 비공개 저장소를 사용합니다.
...
`--reset`은 원본이 없으면 실행을 거부합니다. 임시 DB를 생성해 무결성을 확인하고 기존 DB를 자동 백업한 뒤에만 교체하지만, 사용자 입력 데이터가 있다면 먼저 화면의 `데이터·백업`에서 별도 내보내기도 받아두세요.
...
- 공개 접속에서는 대시보드와 운영자료를 열람하고, 요청에 따라 팀 확인 게시글·댓글·진행 상태를 누구나 작성할 수 있습니다. 행사·매뉴얼·운영 기록 수정은 `TEAM_ACCESS_PIN`, 동기화·백업·보관·복원·DB 관리는 `ADMIN_ACCESS_PIN`이 필요합니다.
- 접근번호는 Streamlit Secrets에만 저장하며 GitHub 파일에는 기록하지 않습니다. 5회 실패하면 10분간 다시 시도할 수 없고, 로그인 권한은 1시간 뒤 만료됩니다. 이전 `REVIEW_BOARD_DELETE_PIN`은 관리자 전체 권한으로 사용하지 않습니다.
