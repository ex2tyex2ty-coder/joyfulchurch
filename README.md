...
동기화는 참석인원·라인업·원본 매뉴얼을 갱신하지만, 앱에서 작성한 결정사항과 운영로그는 삭제하지 않습니다.
...
2. OAuth 동의 화면을 설정하고 `데스크톱 앱` 유형의 OAuth 클라이언트 JSON을 발급합니다.
3. 앱의 `교회력` 메뉴에서 Calendar ID와 JSON 파일을 저장합니다.
4. `Google Calendar 연결·동기화`를 누르고 읽기 전용 권한에 동의합니다.

인증 파일과 토큰은 `data/` 아래 로컬에만 저장되며 Git에서 제외됩니다. 공식 설정 절차는 [Google Calendar Python Quickstart](https://developers.google.com/workspace/calendar/api/quickstart/python)를 참고하세요.
...
자동 테스트는 요청된 A~F 시나리오와 8개 메뉴의 UI 렌더링을 확인합니다.
...
- V1에는 로그인·복잡한 권한이 없습니다. 공용 인터넷에 직접 노출하지 말고 개인 PC 또는 사설망에서 사용하세요.
