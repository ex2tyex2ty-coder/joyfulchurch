...
`성경 검색`은 `창:1:1`, `창세기 1장 1절`, `행 7:2~3`, `사도행전 7장 2~3절` 형식과 여러 구절이 들어간 설교 문자를 인식합니다. `~`, `-`, `–`로 표시된 같은 장의 구절 범위는 각 절로 자동 확장합니다. `창1:1 본문` 형식의 성경 전체 TXT를 현재 세션의 검색 자료로 연결하거나, 설교 문자·구절 목록 TXT를 별도로 불러올 수 있습니다. UTF-8과 윈도우 한글 메모장 형식을 지원하며 한 번에 최대 100개 구절을 처리합니다. 구절 표기 인식은 별도 설정 없이 작동하며, 전체 본문 파일을 사용하지 않을 때 번역문을 표시하려면 [API.Bible](https://scripture.api.bible/)에서 발급한 API 키를 Streamlit Secrets에 저장합니다.

성경 전체 TXT는 서버 저장소나 배포 폴더에 복사하지 않고 현재 브라우저 세션에서만 읽습니다. 공개 앱에서 번역문을 표시할 때는 해당 번역본의 저작권 이용 허락 범위를 확인해야 합니다.

```toml
API_BIBLE_KEY = "발급받은-API-키"
```

키에 연결된 한국어 번역본이 여러 개면 화면에서 선택할 수 있습니다. 항상 같은 번역본을 기본으로 사용하려면 해당 Bible ID도 함께 저장합니다.

```toml
API_BIBLE_ID = "사용할-한국어-Bible-ID"
```

API 키는 브라우저에 노출하지 않고 서버에서만 사용합니다. 표시되는 번역문과 저작권 안내는 API 계정에 허용된 번역본을 따르며, 본문 조회 시 API.Bible의 FUMS v3 조회 기록도 함께 전송합니다.
...
joyful_worship_ops/
├─ app.py                  # Streamlit UI
├─ bible_lookup.py         # 한국어 성경 표기 인식·정식 API 본문 조회
├─ config.py               # 경로와 상태 상수
...
- V1은 개인별 출석을 추적하지 않고 예배별 집계만 사용합니다.
